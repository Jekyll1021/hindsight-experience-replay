...
class ddpg_agent:
    def __init__(self, args, env, env_params, normalize=True):
        ...
        self.actor_target_network = actor(env_params, env_params['obs'] + env_params['goal'], ee_pose=(not normalize))
        self.critic_target_network = critic(env_params, env_params['obs'] + env_params['goal'] + env_params['action'])
    ...
    def _update_network(self):
        ...
        if self.normalize:
            ...
            obs_norm = transitions['obs'].copy()
            g_norm = transitions['g'].copy()
            inputs_norm = np.concatenate([obs_norm, g_norm], axis=1)
            obs_next_norm = transitions['obs_next'].copy()
            g_next_norm = transitions['g_next'].copy()
            inputs_next_norm = np.concatenate([obs_next_norm, g_next_norm], axis=1)
