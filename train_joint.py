...
def launch(args):
    ...
    env_lst = [gym.make(name, reward_type='sparse', goal_type='random', cam_type='fixed', gripper_init_type='random', act_noise=True, obs_noise=True) for name in env_name_lst]
    # get the environment parameters
    env_params = get_env_params(env_lst[0])
    # create the ddpg agent to interact with the environment
    ddpg_trainer = ddpg_joint_ee_agent(args, env_lst, env_params, expert_model_path_lst)
    ddpg_trainer.learn()
